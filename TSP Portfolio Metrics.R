...
portfolio <- read_excel("/Users/2020macbookpro/Downloads/On Course Capital LLC/Portfolio Metrics/TSP PORTFOLIO.xlsx")
...
net_invest_25_per_customer <- schedule %>%
  group_by(name) %>%
  mutate(march_2025_buyout = replace_na(march_2025_buyout, 0)) %>%
...
net_invest_26_per_customer <- schedule %>%
  group_by(name) %>%
  mutate(march_2026_buyout = replace_na(march_2026_buyout, 0)) %>%
...
write_xlsx(data_list, path = "TSP Metrics.xlsx")
...
saveWorkbook(wb, file = "TSP Metrics.xlsx", overwrite = TRUE)
#saves workbook
metrics <- loadWorkbook("TSP Metrics.xlsx")
names <- names(metrics)
for (sheet in names) {
...
saveWorkbook(metrics, file = "TSP Metrics.xlsx", overwrite = TRUE)
#resaves workbook
system("open /Users/2020macbookpro/TSP Metrics.xlsx")
#opens workbook
