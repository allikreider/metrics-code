...
portfolio <- read_excel("/Users/2020macbookpro/Downloads/On Course Capital LLC/Portfolio Metrics/LU PORTFOLIO.xlsx")
...
schedules_per_customer <- schedule %>%
  group_by(name) %>% 
  summarise(schedules = n()) 
#find schedules per customer
volume_per_customer <- schedule %>%
  group_by(name) %>%
  summarise(volume = sum(full_schedule_funding))
#find volume per customer
net_invest_25_per_customer <- schedule %>%
  group_by(name) %>%
  drop_na(march_2025_buyout) %>%
  summarise(net_investment_2025 = sum(march_2025_buyout))
#find net investment 2025 per customer
net_invest_26_per_customer <- schedule %>%
  group_by(name) %>%
  drop_na(march_2026_buyout) %>%
  summarise(net_investment_2026 = sum(march_2026_buyout))
#find net investment 2026 per customer
assets_per_customer <- asset %>%
  group_by(name) %>%
  summarise(assets = n())
#find assets per customer

table1 <- merge(schedules_per_customer,volume_per_customer)
table1 <- merge(table1, net_invest_25_per_customer)
table1 <- merge(table1, net_invest_26_per_customer)
table1 <- merge(table1, assets_per_customer)
#merge all datasets
...
customers_22_23 <- customers_22 %>%
  ...
  summarise(repeat_rate_22_23 = n()/length(customers_22$name))
#finds repeat rate for 2022-2023
...
#for new repeat rates, copy previous lines (249-257) and change dataset names and filters to accomodate correct years
...
write_xlsx(data_list, path = "LU Metrics.xlsx")
...
addStyle(wb, sheet = "Sheet8", style = percentage_format, cols = 9:10, rows = 2:(nrow(table8) + 1), gridExpand = TRUE)
...
saveWorkbook(wb, file = "LU Metrics.xlsx", overwrite = TRUE)
#saves workbook
metrics <- loadWorkbook("LU Metrics.xlsx")
names <- names(metrics)
for (sheet in names) {
...
saveWorkbook(metrics, file = "LU Metrics.xlsx", overwrite = TRUE)
#resaves workbook
system("open /Users/2020macbookpro/LU Metrics.xlsx")
#opens workbook
